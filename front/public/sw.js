importScripts("/buildClient/wb-manifest.522a11cc6fc022ab4cee25fc50f372ac.js", "https://storage.googleapis.com/workbox-cdn/releases/4.3.1/workbox-sw.js");
...
if (workbox) {
	(function () {
		...
		/* Static caching */
		workbox.routing.registerRoute(/^(https|http):\/\/((www)\.mainapp\.com|localhost:3003)\/.*\.(woff|woff2|png|jpg|jpeg|svg|gif|ico).*/, new workbox.strategies.CacheFirst({
			cacheName: ASSETS_CACHE,
			plugins: [new workbox.expiration.Plugin({
				maxEntries: 50,
		...
		workbox.routing.registerRoute(/^(https|http):\/\/((www)\.mainapp\.com|localhost:3003)\/.*\.(js|css).*/, new workbox.strategies.StaleWhileRevalidate({
			cacheName: ASSETS_CACHE,
			plugins: [new workbox.expiration.Plugin({
				maxEntries: 50,
				maxAgeSeconds: 30*24*60*60, // 30 days
		...
		workbox.routing.registerRoute(/^(https|http):\/\/(.*(?<!im)\.mainapp\.com|localhost:3003)\/.*/, function (args) {
